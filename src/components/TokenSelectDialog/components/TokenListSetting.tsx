...
import { JupTokenType } from '@solswap.cloud/raydium-sdk-v2'
...
export default function TokenListSetting({ onClick }: { onClick: () => void }) {
  ...
  const isSolSwapTokenListSwitchOn = useAppStore((s) => s.displayTokenSettings.official)
  ...
  return (
    <Box height="50vh">
      <TokenListRowItem
        name={`SolSwap ${t('common.token_list')}`}
        tokenCount={raydiumTokenListTokenCount}
        isOpen={isSolSwapTokenListSwitchOn}
        switchable={false}
      />
