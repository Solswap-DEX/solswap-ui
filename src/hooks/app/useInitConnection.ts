...
import { TxVersion, validateAndParsePublicKey, txToBase64 } from '@solswap.cloud/raydium-sdk-v2'
...
function useInitConnection(props: SSRData) {
  ...
  const { urlConfigs, fetchRpcsAct, initSolSwapAct, raydium } = useAppStore(
    (s) => ({
      urlConfigs: s.urlConfigs,
      fetchRpcsAct: s.fetchRpcsAct,
      initSolSwapAct: s.initSolSwapAct,
      raydium: s.raydium
    }),
    shallow
  )
  ...
  useEffect(() => {
    ...
    initSolSwapAct({ connection, ...ssrReloadData })
    // eslint-disable-next-line
  }, [initSolSwapAct, connection?.rpcEndpoint, raydium, signAllTransactions, isNeedReload])
